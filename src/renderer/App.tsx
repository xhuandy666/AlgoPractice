...
const pages: { id: Page; label: string; icon: IconName }[] = [
  { id: 'today', label: '今日复习', icon: 'archive' },
...
export function App() {
  ...
  async function setPage(next: Page) {
    ...
    try { await flushPendingSaves(); setPageRaw(next); return true; } catch (error) { setError(errorText(error)); return false; }
  ...
  useEffect(() => {
    ...
    const pulse = () => { if (!editsFrozen() && document.visibilityState === 'visible' && document.hasFocus() && performance.now() - lastInput < 300000) void api.activityPulse(practice.workspace!.attempt!.id).catch(() => {}); }; pulse(); const timer = setInterval(pulse, 10000);
    return () => { clearInterval(timer); for (const name of ['keydown', 'pointerdown', 'wheel']) window.removeEventListener(name, input); };
  }, [api, page, practice.workspace?.attempt?.id]);
  ...
  async function run() {
    ...
    const request = crypto.randomUUID(); runId.current = request; runCancelled.current = false; setRunning(true); setRunEvent(null); setError(''); setNotice('');
    try {
  ...
  async function finish() {
    if (running || editsFrozen()) return; await flushPendingSaves(); const ended = await practice.finish(); if (ended) { setEndedAttempt(ended); setNotice(ended.lastRunMatchesFinal ? '本次练习已归档。' : '本次练习已归档；最终代码尚未运行，已在档案中标明。'); await refreshLibrary(); }
  }
  const commands = [
  ...
  return <div className="app-shell app-shell-p2">
    <aside className="sidebar"><a className="wordmark" href="#library" onClick={event => { event.preventDefault(); setPage('library'); }}><span className="brand-symbol">a.</span><span>题序</span></a>
      ...
      <div className="library-nav"><h2>快捷题目</h2>{library.problems.slice(0, 4).map(problem => <button key={problem.id} className={practice.target.id === problem.id ? 'selected' : ''} onClick={() => openProblem(problem.id)}>{problem.content.title}</button>)}</div>
      <div className="sidebar-note"><span>专注，一道一道来。</span><p>题目与练习留在本机。<br />准备好内容后，离线也能继续。</p></div><div className="prototype-label">AlgoPractice · 本机练习</div>
    </aside>
    <main className="main-shell"><header className="app-header"><div><span className="breadcrumb">{page === 'workbench' && problem ? `${sourceLabel(problem.source)} / ${problem.tags[0] || '算法练习'}` : '个人算法练习'}</span><h1>{pages.find(item => item.id === page)?.label}</h1></div><span className="local-label">本机存储</span></header>
      ...
      {page === 'today' && <TodayPage api={api} library={library} onOpen={id => { void openProblem(id); }} onReview={item => { void openProblem(item.problemId, item.language === 'none' ? practice.target.language : item.language).then(() => setReviewItem(item)); }} onSettings={() => { void setPage('learning-settings'); }} onError={setError} />}
      ...
      {page === 'archives' && <ArchivePage api={api} onError={setError} onRestore={async archive => { if (!api) return; await practice.flush(); const restored = await api.restoreRun(archive.id, crypto.randomUUID()); await openProblem(restored.draft.problemId, restored.draft.language, restored.draft.scopeId); setNotice('已从历史运行恢复独立草稿，普通练习草稿仍保留。'); }} />}
      ...
      {page === 'workbench' && (problem ? <>
        ...
        <div className="workbench-toolbar"><div className="topic-line"><span>{difficultyLabel(problem.difficulty)}</span><span>{problem.mode === 'function' ? '函数题' : 'ACM'}</span><span>{available?.label}</span></div><div className="view-controls"><button aria-pressed={showStatement} onClick={() => setShowStatement(!showStatement)}>{showStatement ? '收起题面' : '展开题面'}</button><button aria-pressed={showHistory} onClick={() => setShowHistory(!showHistory)}>{showHistory ? '收起记录' : '展开记录'}</button>{workspace?.attempt ? <button disabled={frozen || running || practice.switching || !practice.ready} onClick={() => { void finish().catch(error => setError(errorText(error))); }}>结束练习</button> : <button disabled={!api || practice.switching || !practice.ready} onClick={() => { void openProblem(practice.target.id, practice.target.language, practice.target.scope); }}>开始新练习</button>}</div></div>
        ...
        <div className="workbench workbench-p2" data-statement={showStatement} data-history={showHistory} style={{ '--statement-size': `${statementWidth}%`, '--history-size': `${panel === 'ai' ? Math.max(300, historyWidth) : historyWidth}px` } as CSSProperties}>
          ...
          <section className="coding-pane" aria-label="代码与测试"><div className="editor-toolbar"><label className="language-label"><span className="sr-only">编程语言</span><select aria-label="编程语言" value={practice.target.language} disabled={!practice.ready || practice.switching || running} onChange={event => { void openProblem(practice.target.id, event.target.value as Language, practice.target.scope); }}><option value="python">Python</option><option value="java">Java</option></select></label><span className={practice.saving === '保存失败' ? 'save-state error-text' : 'save-state'} role="status">{practice.saving}</span>{practice.saving === '保存失败' && <button className="text-button" onClick={() => { void practice.flush().catch(() => {}); }}>重试保存</button>}<div className="run-actions">{running ? <button className="button primary" onClick={() => { runCancelled.current = true; void api?.cancel().catch(error => setError(errorText(error))); }}><Icon name="stop" />停止</button> : <button className="button primary" disabled={!api || !practice.ready || practice.switching || !available?.canRun} onClick={() => { void run(); }}><Icon name="play" />运行</button>}</div></div>
            ...
            <section className="results-pane" aria-label="测试结果"><div className="results-heading"><h3>测试结果</h3><span>{running ? `${phase}${runEvent?.caseIndex === undefined ? '' : ` · 用例 ${runEvent.caseIndex + 1}`}` : `${problem.cases.length} 个本地用例`}</span></div>
