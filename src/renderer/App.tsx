...
export function App() {
  ...
  const [statementWidth, setStatementWidth] = useState(27); const [historyWidth, setHistoryWidth] = useState(300);
  ...
  return <div className="app-shell app-shell-p2" data-page={page}>
    <aside className="sidebar"><a className="wordmark" href="#today" onClick={event => { event.preventDefault(); setPage('today'); }}><span className="brand-symbol">t.</span><span>题炼</span></a>
      ...
      <nav aria-label="主导航">{pages.map(item => <button key={item.id} data-group-start={item.id === 'sources' || item.id === 'learning-settings' || undefined} aria-current={page === item.id ? 'page' : undefined} onClick={() => setPage(item.id)}><Icon name={item.icon} /><span>{item.label}</span></button>)}</nav>
      <div className="sidebar-footer"><span className="local-indicator" />本地练习 · 随时继续</div>
    </aside>
    <main className="main-shell"><header className="app-header"><div><span className="breadcrumb">{page === 'workbench' && problem ? `${sourceLabel(problem.source)} / ${problem.tags[0] || '算法练习'}` : '题炼 / 我的学习'}</span><h1>{pages.find(item => item.id === page)?.label}</h1></div>{page === 'workbench' && problem ? viewControls : <span className="local-label"><span className="local-indicator" /> 本地空间</span>}</header>
      ...
      {workbenchMounted && <div className="workbench-screen" hidden={page !== 'workbench'}>{problem ? <>
        ...
        <div className="workbench workbench-p2" data-statement={showStatement} data-history={showHistory} style={{ '--statement-size': `${statementWidth}%`, '--history-size': `${panel === 'ai' ? Math.max(300, historyWidth) : historyWidth}px` } as CSSProperties}>
          {showStatement && <><section className="statement" aria-label="题目描述"><div className="statement-heading"><span>题目</span><button aria-label="收起题目描述" onClick={() => { setShowStatement(false); statementToggle.current?.focus(); }}>收起</button></div><h2>{problem.title}</h2><div className="statement-meta"><span className="problem-difficulty">{difficultyLabel(problem.difficulty)}</span><span>{problem.mode === 'function' ? '函数题' : 'ACM'}</span>{problem.sourceUrl && <button className="text-button" disabled={!api} onClick={() => api?.openSource(problem.sourceUrl!).catch(error => setError(errorText(error)))}>打开原站 ↗</button>}</div><Statement content={problem} /><div className="statement-footnote"><strong>{available?.label}</strong><p>{available?.reason}</p></div></section><Splitter side="statement" value={statementWidth} onChange={setStatementWidth} /></>}
