...
export function App() {
  ...
  async function openOfficialJudge() {
    const url = practice.workspace?.problem.content.sourceUrl;
    if (!api || !url || !practice.ready || editsFrozen()) return;
    try { await api.copyCode(practice.code); await api.openWebLink(url); setNotice('代码已复制。在力扣粘贴并提交，即可获得官方判题结果。'); }
