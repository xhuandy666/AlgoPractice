...
export function AiPanel({ api, attempt, code, selectedRun, onApply, onNoteSaved, onSettings, onError, flush, reviewMode = false, active = true }: {
  ...
  async function ask(kind: AiKind = 'hint') {
    if (!api || !attempt || !canAsk || working.current) return;
    ...
    try {
      ...
      const record = await api.askAi({ requestId, attemptId: currentAttempt, kind, question: question.trim(),
        ...(useSelectedRun && selectedRun?.attemptId === currentAttempt ? { runId: selectedRun.id } : {}), noteIds,
        conversationIds: includeHistory ? requests.filter(previous => previous.status === 'completed').slice(-4).map(previous => previous.id) : [] });
      setRequests(previous => [...previous.filter(item => item.id !== record.id), record]); setQuestion('');
