...
export function ArchivePage({ api, initialLearningDate, onRestore, onError }: { initialLearningDate?: string; api: DesktopBridge | undefined; onRestore: (run: RunArchive) => Promise<void>; onError: (message: string) => void }) {
  ...
  const [language, setLanguage] = useState('all'); const [from, setFrom] = useState(''); const [to, setTo] = useState(''); const [refresh, setRefresh] = useState(0);
  ...
  const [official, setOfficial] = useState<{ attemptId: string; records: OfficialSubmission[] } | null>(null);
  useEffect(() => {
    let alive = true;
    if (selected && api) void api.officialSubmissions(selected).then(records => { if (alive) setOfficial({ attemptId: selected, records }); }).catch(error => { if (alive) onError(errorText(error)); });
    return () => { alive = false; };
  ...
  useEffect(() => { const refresh = () => { if (!deleting) setRefresh(value => value + 1); }; const remove = api?.onLibraryChanged(refresh); const end = api?.onMaintenanceEnd(refresh); return () => { remove?.(); end?.(); }; }, [api, deleting]);
  ...
  useEffect(() => {
    ...
    const filters: AttemptPageFilter = { offset: archiveOffset, limit: ATTEMPT_PAGE_SIZE, ...(learningDate ? { learningDate, timeZone } : {}),
      ...
      ...(language !== 'all' ? { language: language as 'python' | 'java' } : {}),
    ...
    return () => { alive = false; };
  }, [api, archiveOffset, query, filter, language, dateFilter, refresh, learningDate, timeZone]);
  ...
  return <section className="archives-page scroll-page"><div className="page-intro"><div><h2>每一次练习，都有存档</h2><p>题面、代码和测试结果保留当时的版本。恢复历史会另建草稿。</p></div><span className="muted">{archives.total} 次练习</span></div>
    ...
    <div className="archive-layout"><div className="archive-list" aria-label="练习列表" aria-busy={loading}>{archives.items.map(row => <button key={row.attempt.id} className={selected === row.attempt.id ? 'selected' : ''} disabled={deleting || restoring || loading} onClick={() => selectArchive(row.attempt.id)}><span className="archive-title">{row.title}</span><span>{row.attempt.language === 'python' ? 'Python' : 'Java'} · {dateTime(row.attempt.startedAt)}</span><span>{row.attempt.isActive ? '进行中' : '已结束'} · {row.runCount} 次运行 · {Math.round((row.activeMs ?? 0) / 60000)} 分钟{row.helpLevel ? ' · 使用过 AI' : ''}</span>{!row.attempt.isActive && row.attempt.lastRunMatchesFinal === false && <span className="warning-text">最终代码尚未本地运行</span>}</button>)}{!archives.items.length && <div className="empty-state"><h3>{loading ? '正在读取档案…' : '还没有匹配的练习'}</h3><p>在工作台开始练习并运行代码后，记录会出现在这里。</p></div>}</div>
      <section className="archive-detail" aria-label="档案详情">{detail ? <><div className="section-heading"><div><h3>{content?.title || detail.attempt.problemId}</h3><p>{dateTime(detail.attempt.startedAt)}{detail.attempt.endedAt ? ` → ${dateTime(detail.attempt.endedAt)}` : ' · 进行中'}</p></div><span className="muted">{detail.attempt.language === 'python' ? 'Python' : 'Java'}</span></div>
        {!detail.attempt.isActive && <div className="archive-conclusion" role="status"><strong>{detail.attempt.lastRunMatchesFinal ? '最终代码与最后一次本地运行一致' : '最终代码尚未本地运行'}</strong><p>{detail.attempt.lastRunMatchesFinal ? '测试结果仅对应这里保存的代码和用例。' : '最后一次测试的结果不能代表最终代码。归档内容不会因后续修改而改变。'}</p></div>}
        ...
        {official?.attemptId === selected && official.records.length > 0 && <details className="archive-learning" open><summary>力扣官方提交 · {official.records.length} 次</summary><OfficialJudgePanel key={selected} records={official.records} currentCode={detail.attempt.finalCode ?? undefined} problemVersion={detail.attempt.problemVersion} busy={frozen} onResume={id => { void api?.officialResume(id).catch(error => onError(errorText(error))); }} onLogin={() => { void api?.loginSource().catch(error => onError(errorText(error))); }} onOpen={url => { void api?.openWebLink(url).catch(error => onError(errorText(error))); }} /></details>}
