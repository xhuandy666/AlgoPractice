...
export function AiSettings({ api, onError }: { api: DesktopBridge | undefined; onError: (message: string) => void }) {
  ...
  async function clearKey() {
    if (!api || working.current || frozen) return; working.current = true; setBusy('clear');
  ...
  return <section className="settings-section" aria-labelledby="ai-settings-title"><div><h3 id="ai-settings-title">AI 接口</h3><p>使用你自己的兼容接口与模型。主动请求帮助时，会发送当前题面、代码、相关运行和你选中的笔记。</p><p>Key 不进入普通设置、日志或备份，界面只显示是否配置。</p></div>
    <form className="p3-form" onSubmit={event => { event.preventDefault(); void save(); }}>
      <label>模型提供商<select aria-label="模型提供商" value={selectedPreset?.id ?? 'custom'} disabled={Boolean(busy) || frozen} onChange={event => { setForm(event.target.value === 'custom' ? { ...form, compatibility: 'openai-compatible' } : createAiProviderPreset(event.target.value, crypto.randomUUID())); setConnection(null); setMessage(''); }}>
      ...
      <label>接口地址<input type="url" required value={form.baseUrl} placeholder="https://你的提供商/v1" disabled={Boolean(busy) || frozen} onChange={event => update('baseUrl', event.target.value)} /></label>
      <label>模型名称<input required value={form.model} placeholder="填写提供商支持的模型名称" disabled={Boolean(busy) || frozen} onChange={event => update('model', event.target.value)} /></label>
      <label>API Key<input type="password" autoComplete="off" spellCheck={false} value={key} disabled={Boolean(busy) || frozen || provider?.secureStorageAvailable === false} placeholder={provider?.hasKey ? '已保存；留空保留，输入可替换' : '粘贴后点击保存'} onChange={event => { setKey(event.target.value); setMessage(''); }} /></label>
      ...
      <details><summary>模型参数与兼容选项</summary><div className="p3-form">
        <label>适配协议<select aria-label="适配协议" value={form.compatibility ?? 'openai-compatible'} disabled={Boolean(busy) || frozen} onChange={event => { update('compatibility', event.target.value as AiProviderConfig['compatibility']); if (event.target.value === 'glm' && form.temperature > 1) update('temperature', 1); }}><option value="deepseek">DeepSeek</option><option value="glm">GLM</option><option value="qwen">Qwen</option><option value="openai-compatible">通用 OpenAI 兼容</option></select></label>
        <div className="form-columns"><label>温度<input type="number" min="0" max={form.compatibility === 'glm' ? '1' : '2'} step="0.01" value={form.temperature} disabled={Boolean(busy) || frozen} onChange={event => update('temperature', Number(event.target.value))} /></label><label>最多输出 Token<input type="number" min="256" max="8192" step="256" value={form.maxOutputTokens} disabled={Boolean(busy) || frozen} onChange={event => update('maxOutputTokens', Number(event.target.value))} /></label></div>
        <label>请求超时（秒）<input type="number" min="5" max="180" value={form.timeoutMs / 1000} disabled={Boolean(busy) || frozen} onChange={event => update('timeoutMs', Number(event.target.value) * 1000)} /></label>
        <label className="checkbox-field"><input type="checkbox" checked={form.jsonMode} disabled={Boolean(busy) || frozen} onChange={event => update('jsonMode', event.target.checked)} />提供商支持 JSON 模式</label>
        <label className="checkbox-field"><input type="checkbox" checked={form.includeUsage} disabled={Boolean(busy) || frozen} onChange={event => update('includeUsage', event.target.checked)} />请求流式用量信息</label>
        <p className="field-help">若提供商拒绝可选参数，可关闭后重新测试。用量未返回时显示未知。</p>
      </div></details>
      <div className="compact-actions"><button type="submit" className="button primary" disabled={!api || Boolean(busy) || frozen || !dirty || !form.baseUrl.trim() || !form.model.trim()}>{busy === 'save' ? '正在保存…' : '保存 AI 设置'}</button><button type="button" className="button" disabled={!provider?.hasKey || Boolean(busy) || frozen || dirty} onClick={() => { void testConnection(); }}>{busy === 'test' ? '正在测试…' : '测试连接'}</button><button type="button" className="text-button" disabled={!provider?.hasKey || Boolean(busy) || frozen} onClick={() => { void clearKey(); }}>清除 Key</button></div>
