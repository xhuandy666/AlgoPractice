...
function ActivityHeatmap({ dashboard, onArchives }: { dashboard: LearningDashboard; onArchives: (date?: string) => void }) {
  ...
  return <div className="activity-history">
    <div className="section-heading"><div><h3>学习轨迹</h3><p>每天完成的题数</p></div><div className="segmented-control" aria-label="学习轨迹时间范围"><button aria-pressed={range === 26} onClick={() => setRange(26)}>半年</button><button aria-pressed={range === 52} onClick={() => setRange(52)}>一年</button></div></div>
    <div className="heatmap-scroll"><div className={`heatmap-chart heatmap-${range}`} style={{ '--heat-columns': range } as CSSProperties}>
    ...
    <div className="heatmap-footer"><span><strong>{dashboard.totals.activeDays}</strong> 个学习日 <span className="muted">· 近一年</span></span><div className="heatmap-legend" aria-label="颜色越深，完成题数越多"><span>少</span>{[0, 1, 2, 3, 4].map(level => <i key={level} data-level={level} />)}<span>多</span></div></div>
  </div>;
}
...
export function TodayPage({ api, library, onReview, onSettings, onArchives, onBrowse, onError }: {
  ...
  return <section className="learning-center scroll-page" aria-busy={loading}>
    <div className="center-heading"><div><p className="center-date">{new Intl.DateTimeFormat('zh-CN', { timeZone: 'UTC', month: 'long', day: 'numeric', weekday: 'long' }).format(new Date(`${today}T12:00:00Z`))}</p><h2>今日进度</h2></div><button className="button primary" onClick={onBrowse}><Icon name="play" />开始练习</button></div>
    <section className="activity-panel" aria-label="学习进度">
      <div className="daily-focus"><div className="section-heading"><h3>今日目标</h3><button className="text-button" disabled={!api || !dashboard || !!busy || frozen} onClick={() => { setGoal(String(target)); setGoalOpen(!goalOpen); }}>调整目标</button></div>
        {goalOpen ? <form className="goal-form" onSubmit={event => { event.preventDefault(); void saveGoal(); }}><label>每天完成<input aria-label="每日练习目标" type="number" min="1" max="1000" value={goal} onChange={event => setGoal(event.target.value)} autoFocus />题</label><div className="compact-actions"><button className="button primary" disabled={!!busy}>保存</button><button type="button" className="text-button" onClick={() => setGoalOpen(false)}>取消</button></div></form> : <>
          <div className="goal-progress" role="img" aria-label={`今日完成 ${day.completedProblems} 题，目标 ${target} 题`}><svg viewBox="0 0 160 160" aria-hidden="true"><circle className="goal-track" cx="80" cy="80" r="66" /><circle className="goal-value" cx="80" cy="80" r="66" pathLength="100" strokeDasharray={`${progress * 100} 100`} /></svg><div><strong>{dashboard ? day.completedProblems : '—'}</strong><span>/ {target} 题</span></div></div>
          <p className={`goal-caption ${progress === 1 ? 'goal-achieved' : ''}`} title="完成题数按已结束的练习去重统计，不代表力扣官方 AC">{progress === 1 ? '今日目标已达成' : `再完成 ${Math.max(0, target - day.completedProblems)} 题，达成今日目标`}</p>
        </>}
        <div className="daily-details"><button onClick={() => onArchives(today)}><span>今日学习</span><strong>{dashboard ? duration(day.activeMs) : '—'}</strong></button><div title="有学习活动、结束练习或完成复习的连续日期；今天尚未开始时延续至昨天"><span>连续学习</span><strong>{dashboard ? learningStreak(dashboard.days, today) : '—'} <small>天</small></strong></div></div>
      </div>
      {dashboard ? <ActivityHeatmap dashboard={dashboard} onArchives={onArchives} /> : <div className="dashboard-skeleton" aria-label={api ? '正在读取学习记录' : '暂无学习记录'}><h3>学习轨迹</h3><div /><p>{api ? '正在读取学习记录…' : '从第一道题开始，记录你的进步。'}</p></div>}
    </section>
    ...
    <div className="review-planner">
      <section className="review-calendar" aria-label="复习日历"><div className="section-heading"><div><h3>复习日历</h3></div><button className="text-button" onClick={onSettings}><Icon name="settings" />复习设置</button></div>
        ...
        <div className="calendar-grid">{monthGrid(month).map(cell => { const count = plans.get(cell.date)?.length ?? 0; const done = doneByDate.get(cell.date)?.length ?? 0; return <button key={cell.date} className="calendar-day" data-outside={!cell.inMonth} data-today={cell.date === today} aria-pressed={cell.date === selected} disabled={!cell.inMonth || !dashboard} aria-label={`${cell.date}，${count} 项待复习，${done} 项已完成`} onClick={() => pickDate(cell.date)}><span className="calendar-day-number">{Number(cell.date.slice(-2))}</span>{cell.inMonth && <span className="calendar-marks">{count > 0 && <span className="calendar-pending">{count}<span> 待复习</span></span>}{done > 0 && <span className="calendar-completed">✓ {done}</span>}</span>}</button>; })}</div>
        <div className="calendar-legend"><span><i />待复习</span><span><i />已完成</span><button className="text-button" onClick={() => onArchives(selected)}>查看当天学习记录 →</button></div>
      </section>
      <section className="review-todos" aria-label="复习任务"><div className="section-heading"><div><h3>{scope === 'day' ? selected === today ? '今天的复习' : `${Number(selected.slice(5, 7))} 月 ${Number(selected.slice(-2))} 日` : scope === 'paused' ? '已暂停' : scope === 'due' ? '全部到期' : '全部复习'}</h3><p>{scope === 'day' ? `${pending.length} 项待复习 · ${completed.length} 项已完成` : `${pending.length} 个复习任务`}</p></div><select aria-label="复习列表范围" value={scope} onChange={event => { setScope(event.target.value as typeof scope); setTab('pending'); setPage(0); }}><option value="day">当天</option><option value="due">全部到期</option><option value="all">全部计划</option><option value="paused">已暂停</option></select></div>
        ...
        <ul className="todo-list">{visible.map(({ item, event }) => item && <li className={`todo-row ${event ? 'is-complete' : ''}`} key={event?.id ?? item.id}>
          <button className="todo-check" aria-label={event ? `${title(item)}，已完成，查看评分` : `开始复习：${title(item)}`} disabled={Boolean(busy) || frozen || item.suspended} onClick={() => event ? setRecordItem(item.id) : onReview(item)}>{event ? '✓' : <Icon name="play" />}</button>
        ...
        {!rows.length && <div className="todo-empty"><span className="todo-empty-symbol" aria-hidden="true">{tab === 'done' ? '◷' : '✓'}</span><h4>{loading ? '正在读取复习…' : tab === 'done' ? '这一天还没有完成记录' : scope === 'paused' ? '没有暂停的复习' : scope === 'due' ? '暂无到期的复习' : scope === 'all' ? '还没有复习计划' : selected === today ? queue?.remainingBudget === 0 && queue.reviewedToday > 0 ? '今日计划已完成' : '今天暂无复习安排' : '这一天没有复习安排'}</h4><p>{tab === 'done' ? '完成复习并评分后，就会记在这里。' : '可以练一道新题，也可以回看学习记录。'}</p>{tab === 'pending' && <button className="text-button" onClick={onBrowse}>去题库看看 →</button>}</div>}
    ...
    <footer className="center-footer"><span>近一年完成 {dashboard?.totals.completedProblems ?? 0} 道不同题目</span><button className="text-button" onClick={() => onArchives()}>全部练习档案 <span aria-hidden="true">↗</span></button></footer>
  </section>;
}
