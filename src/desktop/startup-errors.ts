...
export function startupFailure(error: unknown, dataDirectory: string): StartupFailure {
  ...
  const text = parts.join('\n'); let kind: StartupFailure['kind'] = 'unknown'; let title = 'AlgoPractice 暂时无法启动'; let action = '保留此数据目录。可重新启动；若仍失败，先复制整个目录，再用匹配版本的应用和已验证备份恢复。';
  if (/Unsupported schema version/i.test(text)) { kind = 'version'; title = '当前版本无法读取这份数据'; action = '数据由不同版本的应用创建。请使用相同或更新的 AlgoPractice；如需回退旧版，先保留整个当前目录，再使用升级前的完整备份。不要删除数据库或让旧版强行打开。'; }
